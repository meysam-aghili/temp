...
class Win_dns_util:
    ...
    def get_current_dns():
        dns = re.findall(r'[0-9]+(?:\.[0-9]+){3}', os.popen("netsh interface ipv4 show dnsservers \"Wi-Fi\"").read())
        return dns
    ...
    @staticmethod
    def local_status() -> bool:
        return set(Win_dns_util.get_shecan_dns_list()) == set(Win_dns_util.get_current_dns())
    
    @staticmethod
    def enable():
        os.system("netsh interface set interface \"Wi-Fi\" disable")
        for i in range(len(shecan_dns)):
            if i == 0:
                os.popen("netsh interface ipv4 set dns \"Wi-Fi\" static {}".format(shecan_dns[i]))
            else:
                os.popen("netsh interface ipv4 add dns \"Wi-Fi\" {} index={}".format(shecan_dns[i],i+1))
    ...
    def disable():
        os.system("netsh interface set interface \"Wi-Fi\" disable")
        os.system("netsh interface ipv4 set dns \"Wi-Fi\" dhcp")
        os.system("netsh interface set interface \"Wi-Fi\" enable")
        pass
...
def local_status():
    if platform=="Linux":
        ...
        return Win_dns_util.local_status()
    else:
        print(f"{platform} is not supported")
